...
void ModifiedPermutohedral::compute (int value_size,float* out, const float* in) const
{
	if (value_size <= 2)
		seqCompute(out, in, value_size, false, false);
	else
		sseCompute(out, in, value_size, false, false);
}
